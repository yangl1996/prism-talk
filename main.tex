...
\titlepage
    ...
    \note{say that mohammad is my advisor}
\end{frame}
...
\begin{frame}
    \frametitle{Bitcoin: high security, low throughput, and long latency}
    \note{The idea that Prism takes, is to first deconstruct the blockchain into
    two functional parts, and then scale each one}
    \begin{itemize}
            \pause
...
\begin{frame}
    ...
    \note{explain the terms when introducing the formulas.}
    \note{we can increase mining rate to improve both, or at least increase the
    block size to improve the throughput}
    \pause
    ...
    \pause
    \begin{block}{Latency}
        Confirmation depth ($k$) $\times$ \alert<4>{Mining rate}
    \end{block}
    \pause
...
\begin{frame}
    ...
    \uncover<4->{
        ...
        \note{end with ``so actually how bad is it'', it turns out to be pretty bad}
\end{frame}
