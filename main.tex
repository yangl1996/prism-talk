...
\date[June 28, 2020]{CS Peers Talk at Peking University}
...
\begin{frame}
    \frametitle{Bitcoin prime}
    \begin{columns}
        \column{.5\textwidth}
        \begin{block}{The blockchain}
            \begin{figure}
                ...
                \begin{tikzpicture}[auto, node distance=3em, >=latex']
                    \node[block](genesis){Genesis};
                    \pause
                    \node[block, below of=genesis](1){A pays B \$10};
                    \draw[->](1) to (genesis);
                    \pause
                    \node[block, below of=1](2){C pays A \$40};
                    \draw[->](2) to (1);
                    \pause
                    \node[block, below of=2](3){\dots};
                    \draw[->](3) to (2);
                \end{tikzpicture}
            \end{figure}
        \end{block}
        \pause
        Link: Hash of the parent block
        \pause
        \column{.5\textwidth}
        \begin{block}{Mining}
            \begin{block}{}
            $\text{Hash}(\text{Block}) \le \text{Threshold}$
            \end{block}

            \pause

            \begin{block}{}
            $\text{Hash}(\text{Block} \vert \text{Nonce}) \le \text{Threshold}$

            Randomly try difference nonces.
...
\begin{frame}
    \note{Say that the instance is not heavy during the demo. Demo: http://t.leiy.me/prism-dashboard}
