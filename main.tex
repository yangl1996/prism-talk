...
\begin{frame}
\titlepage
    \note{start with ``I'm excited to be here to talk about'', say that mohammad is my advisor}
\end{frame}
...
\begin{frame}
    \frametitle{Bitcoin: high security, low throughput, and long latency}
    \note{our solution in this problem is to first}
    \begin{itemize}
            \pause
...
\begin{frame}
    \note{another important aspect of Bitcoin is when do we confirm transactions}
...
\begin{frame}
    ...
    \note{explain the terms when introducing the formulas. we can increase mining rate to improve both, or at least increase the block size to improve the throughput. also say why it increase forking}
    \pause
    ...
    \end{block}
    \uncover<6->{
    \alert{Increasing the mining rate or the block size causes forking}
}
\end{frame}
...
\begin{frame}
    \note{we noticed that in bitcoin, performance is coupled with security. why is that? it is because a block has two roles}
...
\begin{frame}
    ...
    \note{proposing is limited by forking. what if forking does not matter anymore. then bring order}
    \begin{figure}
...
\begin{frame}
    \note{proposer block still has two roles: propose and vote. we want to reduce the confirmation depth k here. one chain? not helpful. 1000 chains? 40\% chance to revert one chain, low prob to reverse half}
